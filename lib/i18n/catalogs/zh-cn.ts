...
export const zhCN = {
  ...
  home: {
    hero: {
      ...
      chatChip: "像 Codex，但更好",
      appChip: "由 Agent 构建的 App",
    },
    agents: {
      eyebrow: "Agent",
      title: ["用你已经在用的 Agent。", "订阅还是你自己的。"],
      lede: "Codex、Claude Code、Kimi Code 与 OpenCode 通过各自的官方 CLI 在 Bottega 里运行，登录的是你自己的账号——每家一条命令，就并排在同一个侧栏里。不转售 token，不加价，也不用再订阅什么。",
      roster: {
        codex: "官方 Codex CLI，带着你的账号、模型与权限阶梯。",
        claude: "你机器上装的那个 Claude Code：你的订阅、它的模型、Plan 模式。",
        kimi: "Kimi Code CLI，用你已有的账号，在同一个侧栏里。",
        opencode: "OpenCode 与你配置好的供应商目录，工作不用搬家。",
        more: "还有更多",
        soon: "接入中",
        moreBody: "Hermes、Pi 与 DeepSeek 接下来接入——每家都是自己的 CLI，绝不是代理。",
      },
      handoff: { eyebrow: "交接", title: "一个 Agent 出计划，另一个来实现。", body: "Claude 梳理问题、写出 Plan，再把它投进一条 Chat 交给 Codex 实现。Codex 做完，结果会自动回到原来那条 Chat——由 Claude 评审，或者交给下一个 Agent。" },
      data: { eyebrow: "数据空间", title: "一个数据空间，所有 Agent 共享。", body: "一个 Agent 记下的，下一个直接读到。Base 把每次对话里的记录都放进同一个本地数据空间，同一份记录可以按表格、图表、画廊或地图查看——换了 Agent，上下文也不会重置。" },
    },
    trust: {
      eyebrow: "本地优先 · 加密同步",
      title: ["数据在你的机器上。", "离开机器就是密文。"],
      lede: "Bottega 把你的工作放在一个打得开的文件夹里，通过官方 CLI 运行 Agent，并以 MIT 协议开源。想同步时再打开，同步的只有密文。",
      claims: [
        { title: "一个文件夹，不是黑盒", body: "Chat、附件、Project、Base、App 源码与 Skills 都是你选定的那个文件夹里的文件——不用 Bottega 也能读，复制一份就是备份。" },
        { title: "官方 CLI，你的账号", body: "每个 Agent 都通过它自己的 CLI 运行，以你的身份登录。Bottega 提供的是工作台，中间从不设代理。" },
        { title: "开源，MIT", body: "碰你数据的代码是公开的。可以读、可以审、可以 fork。" },
        { title: "离开之前先加密", body: "同步默认关闭。打开之后，Chat 用只有你知道的密码封存；服务器只拿得到密文。" },
      ],
      ...
      source: {
        eyebrow: "开源",
        title: "开源。发布你自己的版本。",
        body: "每一行都以 MIT 协议开源，加密部分也不例外。读它、改它、发布你自己的构建。",
        fork: "在 GitHub 上 fork",
        browse: "浏览源码",
        license: "MIT 许可",
        cloned: "已把 Bottega 克隆到 ./Bottega",
        installed: "依赖已安装",
      },
    },
    apps: {
      ...
      dock: {
        eyebrow: "Dock",
        title: "你的 App，有自己的 Dock。",
        body: "Bottega 做的每个 App 都在 Bottega Dock 上，一点就开。它停在 macOS 程序坞上方，不动 Mac 的任何设置；想只留一条栏，也可以让它替代系统程序坞。",
        points: ["一次点击打开任意 Bottega App", "macOS 程序坞保持原样", "访达、下载和 Mac 应用也能放在旁边"],
      },
      widgets: {
        eyebrow: "小组件",
        title: "小组件，自己做。",
        body: "Dock 上也能放小组件，做法和 App 一样：描述一个，Agent 就把它做出来。从 AI 额度开始——各个 Agent 套餐还剩多少——或者让它帮你盯预算、盯这周的训练。",
        cases: ["Codex、Claude 和 Kimi 并排显示", "今日 Token 数与估算费用", "支出追踪器里本月的总支出", "健身日志里的连续训练天数"],
      },
    },
    faq: {
      eyebrow: "常见问题",
      title: "常见问题，都在这里。",
      lede: "还有别的问题？做 Bottega 的人会读每一条 issue。",
      issues: "去 GitHub 提问",
      items: [
        { question: "Bottega 免费吗？", answer: "App 本身免费使用，并以 MIT 协议开源，你用的是自己已经付费的 Agent 套餐。同步在公测期间免费，之后会收费。" },
        { question: "支持哪些 Agent？", answer: "Codex、Claude Code、Kimi Code 和 OpenCode，都通过各自的官方 CLI 运行。接下来是 Hermes、Pi 和 DeepSeek。" },
        ...
        { question: "支持哪些平台？", answer: "macOS、Windows 和 Linux。Bottega Dock 需要 Apple 芯片、macOS 15 或更高版本。" },
      ...
      sections: [
        {
          ...
          paragraphs: [
            "记账、管理库存、整理任务清单，把需求告诉 Agent，它就能在 Base 中建立字段和记录。需要补充或修改时，接着说就好。",
            "你可以继续问：“哪些任务还没完成？”“这个月一共花了多少？”Agent 能直接查询、汇总已有记录，数据就在对话旁边，随时可查。",
          ],
          points: [],
        },
        {
          heading: "一份数据，六种看法",
          paragraphs: ["用表格和列表查看明细，用看板跟进进度，用图表观察趋势，用画廊浏览媒体，用地图查看位置。六种视图始终连接同一份数据。"],
          points: ["筛选、排序和分组，快速找到当前关心的记录。", "用公式计算数值，用关联字段连接记录。", "添加媒体附件，查看每条记录的修改历史。", "导入和导出 CSV、JSON、XLSX 文件。"],
        },
        {
          heading: "让数据随着工作持续积累",
          paragraphs: [
            "一个 Chat 可以有自己的 Base，同一 Project 下的多个 Chat 也可以共用项目的 Base。工作继续推进时，已有记录可以接着用。",
            "App 也能围绕 Base 提供专用界面。日常操作用 App，需要查看和处理背后的记录时，打开 Data 标签即可。",
            "Base 将记录保存在本地。App 的读取、新增、修改、删除和附件操作分别授权；每次更新也会检查数据是否已变化，帮助避免旧修改覆盖新内容。",
          ],
          points: [],
        },
        {
      ...
      stories: [
        {
          ...
          title: "统一入口，原生能力不变。",
          paragraphs: [
            "Bottega 通过官方本地 CLI 运行 Codex、Claude Code、Kimi Code 与 OpenCode，不用额外的 Agent 框架。这有以下几点好处：",
            "1. 使用你已经安装的官方 CLI，无需额外的 Agent 框架。同时不需要额外的订阅或付费。",
            "2. 功能上，Bottega 保留各 CLI 的原生能力，同时提供统一的对话体验。",
          ],
        },
        {
          index: "02 · 对话能力对齐",
          title: "统一的是交互，不是 Agent。",
          paragraphs: [
            "每个 Agent Harness 都有其独特的运行模式，但为了统一用户体验，我们统一了一套用户交互界面。对流式回复、工具状态、Plan 模式、队列消息等功能进行了适配。",
            "以下是各功能适配情况（仍然在持续更新中）：",
          ],
        },
        {
          index: "03 · 跨 Agent 协作",
          title: "不止统一体验，更让 Agent 接力完成工作。",
          paragraphs: [
            "让 Claude 先梳理问题、制定 Plan，再把所需上下文交给 Codex 实现；完成后，结果自动回到原对话，继续评审、修改，或交给下一个 Agent。",
            "每个 Chat 都保留自己的 Agent、工作空间与任务记录。消息可立即执行或排队等待，过程始终可见，结果可追溯、可复用。",
          ],
        },
      ],
    },
  ...
  demo: {
    ...
    apps: {
      items: [
        { name: "设计画布", description: "把想法整理成可编辑的视觉画布" },
        { name: "开发看板", description: "规划开发任务、进度与发现" },
        { name: "支出追踪器", description: "记录支出，查看分类与消费趋势" },
        { name: "健身日志", description: "记录训练，查看肌群负荷" },
      ],
    },
    baseViews: [
    ...
    fitness: {
      ...
      search: "搜索", searchHint: "名称、别名、肌群或器械", bodyPart: "身体部位", muscleRegion: "肌群区域", equipment: "器械", all: "全部", exercises: "1324 个动作", showing: "显示 1–24", groups: ["胸部", "大腿", "背部", "肩部", "手臂"],
    },
    handoff: { sent: "Plan 交给 Codex", back: "结果自动回来" },
    ...
    widgets: { limits: "AI 额度", usage: "AI 用量", spend: "本月支出", streak: "连续训练", leftInWindow: "本窗口剩余", today: "今天", tokens: "Token", estimatedCost: "估算费用约 {cost}", inMonth: "9 月至今", month: "9 月", streakUnit: "天连续", days: "天" },
    chats: {
      releaseNotes: {
    ...
    agentsVisual: {
      ...
      rows: [
        { label: "流式回复", values: ["✅", "✅", "✅", "✅"] },
        { label: "Model / Thinking 选择", values: ["✅", "✅", "✅", "✅"] },
        { label: "工具状态", values: ["✅", "✅", "✅", "✅"] },
        { label: "Plan 模式", values: ["✅", "✅", "✅", "✅"] },
        { label: "队列模式", values: ["✅", "✅", "✅", "✅"] },
        { label: "用户审批功能", values: ["✅", "✅", "✅", "✅"] },
        { label: "@ 功能", values: ["✅", "✅", "✅", "✅"] },
      ...
      chatPlan: "统一导航栏", chatImpl: "导航实现", planLabel: "Plan", planTitle: "导航实现计划",
      relayFrom: "【来自 Section @{name}（source_section_id={id}）】",
      queueItem: "@{name} · 顺带把移动端断点也验一下",
      reviewLine: "计划意图与实现一致，没有阻塞问题。",
      notePlan: "先让 Claude 把问题\n梳理成一份 Plan",
      noteSend: "把要做的事投进\n另一条 Chat 的队列",
