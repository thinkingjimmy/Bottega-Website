...
export const ja = {
  ...
  home: {
    ...
    agents: {
      ...
      handoff: { eyebrow: "ハンドオフ", title: "仕事を次の Agent に渡す。", body: "あるプランの利用枠が尽きた？ 次のターンで別の Agent を選べば、同じ Chat のまま続けられます。あえて分業してもいい：Claude が計画を書き、Codex が実装し、Kimi がテストを回す——ひと跳びごとにサイドバーの Chat が一つ増えるだけで、ワークフローファイルの一段ではありません。", note: "各 Agent は複数のサブエージェントを並列に走らせることもできます。" },
    ...
    source: {
      eyebrow: "オープンソース",
      title: "自分のバージョンを出す。",
      lede: "Bottega は MIT ライセンスです。リポジトリを fork し、好きなように変えて、自分のバージョンを出せます——必要な手順は下の数行だけです。",
      terminalLabel: "Bottega をビルドするターミナルコマンド",
      cloned: "Bottega を ./Bottega にクローンしました",
      installed: "依存関係をインストールしました",
      items: [
        { title: "UI を変える。", body: "どの画面でも作り替えられます。テーマは一つのトークンファイルです。" },
        { title: "Agent を足す。", body: "同じ ACP アダプターで別の CLI をつなげます。" },
        { title: "自分のビルドを出す。", body: "署名し、名前を変え、自分のものとして配布できます。" },
      ],
      license: "MIT ライセンス",
      fork: "GitHub で fork",
    },
