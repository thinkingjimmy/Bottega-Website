...
export const ja = {
  ...
  footer: {
    noteBefore: "Bottega は自由なオープンソースソフトウェアです（",
    noteAfter: "）。",
  ...
  home: {
    hero: {
      ...
      date: "9月1日(火)  9:36",
      chatChip: "CLI と同じ感覚の Chat",
      appChip: "Agent がつくる Apps",
    },
    agents: {
      title: "契約中のすべての Agent を、ひとつのサイドバーに。",
      paragraphs: [
        "Codex、Claude、Kimi、OpenCode はすべて Bottega の中で動きます。すでにインストールした公式 CLI を直接使い、別物として再実装しません。",
        "各 Agent はあなた自身のサブスクリプションで動きます。料金は既存プランに沿ってプロバイダーが請求し、私たちが上乗せすることはありません。",
      ],
    },
    apps: {
      title: "AI ネイティブな Apps をつくる。",
      body: "AI フィットネスコーチ、AI 支出トラッカー、あるいはまったく新しいもの。アイデアを伝えるだけで、Bottega がデータから画面まで、動く App に仕上げます。Bottega には次の 4 つの Apps が付属します。",
    },
    customizable: {
      title: "会話するだけで、どんな App もカスタマイズ。",
      body: "Bottega の App はブラックボックスではなく、編集できるソースを持っています。Edit App を選び、変えたいことを伝えると、Agent がソースを直接更新し、機能・データ・画面を変更します。別のコードエディターは不要です。",
    },
    base: {
      title: "すべての Chat に Base を。",
      body: "Bottega は Base でデータを整理・分析・可視化します。合計、要約、カテゴリ別内訳、傾向を尋ねたら、同じデータを表・チャート・ギャラリー・地図で確認できます。エクスポートは不要です。",
    },
    fork: {
      title: "Fork して、自分のビルドを届ける。",
      body: "Bottega は最初から最後まで MIT License です。リポジトリを Fork し、Agent、ツール、UI を変えて、チーム全体に独自ビルドを配布できます。それでも各メンバーのマシンでローカルファーストに動き、既存のサブスクリプションを使います。",
  ...
  features: {
    ...
    agents: {
      ...
      title: "複数の Agent。ひとつのワークスペース。",
      deck: "Codex、Claude Code、Kimi Code、OpenCode をひとつのローカルワークスペースで実行。公式 CLI、既存のプロバイダーアクセス、共通の会話体験をそのまま使えます。",
    },
    apps: {
      label: "Apps",
    ...
    agentsArticle: {
      stories: [
        ...
        {
          index: "02 · 会話機能の整合",
          title: "ひとつの会話体験。平坦化せずに適応する。",
          paragraphs: [
            "ストリーミング応答、ツール活動、Plan レビュー、質問、権限、モデル操作、キュー済みメッセージはひとつの視覚システムを共有します。Bottega は各バックエンドのプロトコルをそこへ写しますが、能力が同じだとは装いません。",
            "マトリクスは現在の製品契約です。「共有」は Bottega の画面が同じことを表し、Native と Adapted は実装差を示します。実行時にない機能を死んだコントロールとして表示しません。",
          ],
        },
        {
          index: "03 · Agent 間コラボレーション",
          title: "ひとつの Agent から別の Agent へ仕事を渡す。",
          paragraphs: [
            "Claude の会話で Plan を作り、範囲を限定した文脈を Codex の会話へ送り、実装を依頼できます。Codex は結果を元の会話へ返し、レビューを受けられます。",
            "受け渡しには Bottega の永続 Chat 通信ツールを使います。対象 Chat は自分の Agent とワークスペースを保ち、メッセージはそこで開始または待機し、結果は隠れた実行の中に消えず、見えて再利用できる状態で残ります。",
          ],
        },
      ],
    },
  ...
  demo: {
    ...
    apps: { shapes: ["ワークスペース成果物 App · 分離 GUI 画面", "Base App · 9 列 · タスク / 指摘 / 台帳", "Base App · 4 列 · 詳細 / 分析ビュー", "GUI 付き Base App · 筋肉ヒートマップ"] },
    baseViews: [
    ...
    agentsVisual: {
      ...
      rows: [
        { label: "ストリーミング応答", values: ["共有", "共有", "共有", "共有"] }, { label: "ツール活動", values: ["共有", "共有", "共有", "共有"] },
        { label: "Plan とレビュー", values: ["ネイティブ", "ネイティブ", "ネイティブ", "適応"] }, { label: "質問", values: ["共通 UI", "共通 UI", "共通 UI", "共通 UI"] },
        { label: "モデル選択", values: ["完全", "リスト", "リスト", "リスト"] }, { label: "権限", values: ["3 段階", "2 段階", "2 段階", "2 段階"] },
        { label: "ターン中メッセージ", values: ["即時", "即時", "次ターン", "次ターン"] },
      ],
      collaborationLabel: "Claude が計画し、Codex が実装し、永続 Chat の受け渡しで Claude がレビュー", persistentChats: "永続 CHATS", flowTitle: "計画 → 実装 → レビュー", visibleHandoff: "見える受け渡し",
      phases: ["01 · 計画", "02 · 実装", "03 · レビュー"], planSummary: "ナビゲーション実装計画", planItems: ["共有 Header 境界を使う", "モバイル動作を保つ", "静的ルートを検証"],
      filesChanged: "3 ファイルを変更", typecheckPassed: "型チェック成功", implementationComplete: "実装完了", noIssues: "ブロッキング問題なし", reviewMatch: "計画意図と実装が一致しています。", reviewReturned: "レビュー結果を返却",
    },
  },
