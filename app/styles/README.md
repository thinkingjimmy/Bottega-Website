...
现在文件名就是地界，`globals.css` 只剩一份十一行的清单。
...
- **base.css** (163): 基础排版与正文骨架。`.split` 三型从原第 5 节提到这里：
  它是页面的网格，与 `.wrap` 同属「页面怎么分块」，留在正文里只是因为当初
  写在那儿。提前不会出事——那几条在 hero 与 chat 里没有第二处定义。
- **hero/** (2 支): 首屏那一屏。见其 README。
- **apps/** (4 支): Apps 一节四台机器的表面。见其 README。
- **reel.css** (314): Agents 一节那台一镜到底的示意图、它的画框、骨架条 `.sk`、
  四枚 agent 标记与 `.sr-only`。`.sk` 与 `.sr-only` 住在这里是因为它们最早
  为它而生，如今也被 `apps/` 那四台机器消费——要动先看那四支。
- **bands.css** (185): Fork 那条深色带与终端框、站点页脚、changelog 的 `.entry`，
  以及 `≤1300` 的两栏拆分。
- **motion.css** (172): 进场动效、`prefers-reduced-motion` 的总闸、`≤900` 的窄屏版。

## 房规

- **每支不超过 800 行**。最长的是 `hero/chat.css`（777）——它已经贴着线，
  再往里加东西之前先想想该不该另起一支。
